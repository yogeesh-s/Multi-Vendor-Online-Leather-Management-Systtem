            ...
            <div class="card card-outline rounded-0 card-primary shadow">
                <div class="card-body">
                    <div class="list-group">
                        ...
                        <?php 
                        $categories = $conn->query("SELECT * FROM `category_list` WHERE delete_flag = 0 AND status = 1 AND seller_id NOT IN (SELECT id FROM `seller_list` WHERE delete_flag != 0 or status != 1) ORDER BY `name` ASC");
                ...
                <div class="card-body">
                    <div class="container-fluid">
                            ...
                            <div class="col-lg-8 col-md-10 col-sm-12">
                                <form action="" id="search-frm">
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-append"><button class="input-group-text"><i class="fa fa-search"></i></button></div>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <div class="row" id="product_list">
                            ...
                            $products = $conn->query("SELECT p.*, v.shop_name as seller, c.name as `category` FROM `product_list` p INNER JOIN seller_list v ON p.seller_id = v.id INNER JOIN category_list c ON p.category_id = c.id WHERE p.delete_flag = 0 AND p.`status` = 1 AND p.seller_id NOT IN (SELECT id FROM `seller_list` WHERE delete_flag != 0 or status != 1) {$swhere} ORDER BY RAND()");
                            ...
                            <div class="col-lg-4 col-md-6 col-sm-12 product-item">
                                ...
                                <div class="product-img-holder position-relative">
                                    ...
                                    <div class="card-body border-top border-gray border border-1">
                                        <h5 class="card-title text-truncate w-100"><?= $row['name'] ?></h5>
                                        <div class="d-flex w-100">
                                            <div class="col-auto px-0"><small class="text-muted">Seller:&nbsp </small></div>
                                        ...
                                        <div class="d-flex">
                                            <div class="col-auto px-0"><small class="text-muted">Price:&nbsp </small></div>
