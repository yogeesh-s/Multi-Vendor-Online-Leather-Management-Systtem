            ...
            <div id="cart-list">
                ...
                ?>
                    ...
                    <div class="col-12 border p-0">
                        <?php 
                        $vtotal = 0;
                        $products = $conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.user_id = '{$_settings->userdata('id')}' and p.seller_id = '{$vrow['id']}' order by p.name asc");
