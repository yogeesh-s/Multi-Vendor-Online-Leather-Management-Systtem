<div class="col-lg-12 py-5">
    ...
    <div class="contain-fluid">
        <div class="card card-outline card-dark shadow rounded-0">
            <div class="card-body rounded-0">
                <div class="container-fluid">
                    <h3 class="text-center">Artificial Leather</h3>
                    <hr/>
                        <?php include("welcome1.html") ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row" id="product_list">
            <?php 
            $products = $conn->query("SELECT p.*, v.shop_name as seller, c.name as `category` FROM `product_list` p INNER JOIN seller_list v ON p.seller_id = v.id INNER JOIN category_list c ON p.category_id = c.id WHERE p.delete_flag = 0 AND p.`status` = 1 AND p.seller_id NOT IN (SELECT id FROM `seller_list` WHERE delete_flag != 0 or status != 1) AND (p.name LIKE '%Artificial Leather%' or p.description LIKE '%Artificial Leather%' or c.name LIKE '%Artificial Leather%' or v.shop_name LIKE '%Artificial Leather%') ORDER BY RAND() limit 4");
        ...
        <div class="text-center">
            <a href="./?page=products&search=Artificial%20Leather" class="btn btn-large btn-primary rounded-pill col-lg-3 col-md-5 col-sm-12">Explore More Products</a>
        </div>-->
    </div>
</div>