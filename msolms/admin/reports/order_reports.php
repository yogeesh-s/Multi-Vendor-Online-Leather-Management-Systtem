                ...
                <div class="callout callout-primary shadow rounded-0">
                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            $total = 0;
                            $orders = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as user,concat(v.code, '-',v.shop_name) as seller from `order_list` o inner join user_list c on o.user_id = c.id inner join seller_list v on o.seller_id = v.id where date_format(o.date_created,'%Y-%m') = '{$month}' order by unix_timestamp(o.date_created) desc ");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="align-middle px-2 py-1"><?php echo ucwords($row['ccode'].' - '.$row['user']) ?></td>
                                    ...
                                    <td class="text-right align-middle px-2 py-1">₹ <?php echo $row['status']!=5 ? format_num($row['total_amount']):0 ?></td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
                        <tfoot>
