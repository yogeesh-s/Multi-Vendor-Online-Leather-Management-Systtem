...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as user, CONCAT(v.code, '-',v.shop_name) as `seller` from `order_list` o inner join user_list c on o.user_id = c.id inner join seller_list v on o.seller_id = v.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<div class="row">
        ...
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($seller) ? $seller : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">User</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($user) ? $ccode.' - '.$user : '' ?></span></div>
