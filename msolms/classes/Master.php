...
Class Master extends DBConnection {
		...
		if ($check > 0) {
			...
			if ($save) {
				...
				if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
					...
					$unique_filename = $pid . '_' . uniqid() . '.' . $file_ext;
					$fname = 'uploads/products/' . $unique_filename;
	...
	function add_to_cart(){
		...
		$check = $this->conn->query("SELECT * FROM cart_list where product_id = '{$product_id}' && user_id = '{$user_id}'")->num_rows;
		if($check > 0){
			$sql = "UPDATE cart_list set quantity = quantity + {$quantity} where product_id = '{$product_id}' && user_id = '{$user_id}' ";
		}else{
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$sellers = $this->conn->query("SELECT * FROM `seller_list` where id in (SELECT seller_id from product_list where id in (SELECT product_id FROM `cart_list` where user_id ='{$this->settings->userdata('id')}')) order by `shop_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $sellers->fetch_assoc()):
			$data = "";
			while(true){
			...
			$data = "('{$ref_code}','{$this->settings->userdata('id')}','{$vrow['id']}','{$this->conn->real_escape_string($delivery_address)}','{$this->conn->real_escape_string($card_number)}')";
			$sql = "INSERT INTO `order_list` (`code`,`user_id`,`seller_id`,`delivery_address`,`card_number`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.user_id = '{$this->settings->userdata('id')}' and p.seller_id = '{$vrow['id']}' order by p.name asc");
		...
		if(!$has_failed){
			...
			$this->conn->query("DELETE FROM `cart_list` where user_id ='{$this->settings->userdata('id')}'");
		}else{
			$resp['status'] = 'failed';
