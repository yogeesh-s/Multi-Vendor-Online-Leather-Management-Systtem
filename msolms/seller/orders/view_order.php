...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as user from `order_list` o inner join user_list c on o.user_id = c.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
