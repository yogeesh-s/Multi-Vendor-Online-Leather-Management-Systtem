<div class="col-lg-12 py-5">
    ...
    <div class="contain-fluid">
        ...
        <div class="row" id="product_list">
            <?php 
            $products = $conn->query("SELECT p.*, v.shop_name as seller, c.name as `category` FROM `product_list` p INNER JOIN seller_list v ON p.seller_id = v.id INNER JOIN category_list c ON p.category_id = c.id WHERE p.delete_flag = 0 AND p.`status` = 1 AND p.seller_id NOT IN (SELECT id FROM `seller_list` WHERE delete_flag != 0 or status != 1) ORDER BY RAND() limit 4");
